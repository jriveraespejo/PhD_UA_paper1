	...
	\begin{columns}
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				%
				\item $B_{i}=$ block {\small \textcolor{blue}{(will reduce $\sigma_{U_{ik}}$) } }
				%
...
\begin{lhframe}[rhgraphic={\includegraphics[scale=0.65]{select_model2.png}}]
	...
	\begin{itemize}
		...
		{\small (similar to the ``simplest" model)}
		%
		\item similar levels of explained variability across levels and blocks,
		%
		\item but there is ``mild" evidence that some interactions might be prevalent
		%
		\begin{itemize}
			\item different $SI$ means within $HI/CI$, \\
			CMV (row 2 of $aEHS$) vs Connexine 26 (row 3 of $aEHS$)
			%
			\item different $SI$ evolution between $NH$ vs $HI/CI$ children
		\end{itemize}
		%
	\end{itemize}
	%
...
\begin{lhframe}[rhgraphic={\includegraphics[scale=0.50]{contrasts.png}}]
	...
	within the ``interaction" model (\texttt{E\_NC5b3}),
	%
	\begin{itemize}
		%
		\item given the size of the data within groups from combinations of $E$ and $HS$, we cannot reject the null hypothesis
		%
		\item similar result is observed with the $A$ slopes contrast \\
		{\small (but in this case is because the effect is small)}
		%
		\item we decide to \textcolor{blue}{keep} the ``interaction" model \textcolor{blue}{(\texttt{E\_NC5b3})}
		%
	\end{itemize}
	%
\end{lhframe}
...
%
%
\begin{lhframe}[rhgraphic={\includegraphics[scale=0.36]{posterior_predictive_real.pdf}}]
	...
	\begin{itemize}
		...
		\item provides a ``true" $H$ and $SI$ measures
		%
	\end{itemize}
	%
\end{lhframe}
%
%
\begin{lhframe}[rhgraphic={\includegraphics[scale=0.36]{outliers.pdf}}]
	{High leverage observations}
	
	``interaction" model has two high leverage observations, \\
	{\small (observations that ``drag" estimation) }
	%
	\begin{itemize}
		%
		...
		\begin{itemize}
			...
			{\small (almost ``perfect" hearing) }
			%
			\item child $15$ is $HI/CI$, $Am \approx 0.67$ \\
			{\small ($min(A)=5 \rightarrow min(Am)=0$)}
			%
		\end{itemize}
		%
	\end{itemize}
