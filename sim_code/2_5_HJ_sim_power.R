...
# run only once
#
HJpower(power_save=file.path(getwd(), 'sim_chain'), # file_save need to include getwd()
        sim_name='HJ_sim2_power.RData', # file_name need to include '.RData'
...
# load results
load(file.path( getwd(), 'sim_chain', 'HJ_sim2_power.RData'))


# plot results
plot_power(d=par_res, exclude='m_SI', plotN=1)
plot_power(d=par_res, exclude='m_SI', plotN=2)
plot_power(d=par_res, exclude='m_SI', plotN=3)
