...
Finally, the reasons of the conspicuous absence of other variables, deemed relevant by the literature, is extended in the supplementary section \ref{sSA:causal_details}.
...
%###############################
%
Using the DAG described in previous section we developed multiple bayesian probabilistic models \cite{Jaynes_2003}. The models targeted the different ways we could state our research hypothesis, but they were also useful to validate our statistical models and code.
...
\noindent where $\text{BetaProp}(\mu, \theta)$ defined the beta-proportion distribution with parameters $\mu$ and $\theta$ \cite{Figueroa-Zuniga_et_al_2013, Kruschke_2015}. Furthermore, $\bar{H}_{bi}$ represented the average entropy measure nested within blocks ($b=1,\dots,5$) and children ($i=1,\dots,32$), while $M_{ik}$ denoted the degrees of freedom for the beta-proportion distribution, nested within utterances ($k=1,\dots,10$), and possibly nested within children. Additionally, $H^{O}_{bik}$, $H^{T}_{i}$ and $SI_{i}$ were defined as in the previous section, $\alpha_{b}$ represented the block random effects, and $\text{logit}^{-1}(x) = \exp(x) / ( 1 + \exp(x) )$. We can notice in the distribution that $\mu = \bar{H}_{bi}$, indicating we try to model the average entropy measure of the replicates, while we use $\theta = M_{ik}$ to capture the variability present in the entropy replicates. See supplementary section \ref{} for a detailed overview on how to model variability in a beta-proportion distribution.
...
\noindent where $A_{i}$ denotes the children's \textit{hearing age}, $E_{i}$ the etiology of the disease that led to the hearing impairment, $HS_{i}$ the hearing status group, and $PTA_{i}$ the post-implant pure tone average.
