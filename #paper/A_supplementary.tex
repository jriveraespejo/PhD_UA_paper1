...
%-------------------------------
%
Figure \ref{fig:BetaProp} shows the implications of different ``sample sizes'' ($M_{ik}$), on the dispersion of the beta-proportion distribution \cite{Kruschke_2015}. The three panels show different assumed ``average'' entropies. The middle panel assumes an ``average'' entropy $\mu=0.5$, the left panel $\mu=0.2$, and the right panel $\mu=0.8$ (as shown by the discontinuous lines).
...
In this sense, we expect that if the posterior estimates for $M_{ik}$ reach lower values, it would imply the entropy replicates $H^{O}_{bik}$ have high dispersion. Therefore, we could say the replicates as a measure of the \textit{true} entropy have low reliability. In contrast, the higher the values posterior estimates for $M_{ik}$, the lower the dispersion, and the higher the reliability of the replicates as a measure for the \textit{true} entropy. 

Finally, this approach also implies that we used the ``sample size'' parameter to model the replicates heterogeneity, effectively estimating a measurement error model \cite{Carroll_2006}.
%
\begin{figure} [!h]
	\centering
...
%
%-------------------------------
\subsubsection{Pre-processing} \label{ssSA:preprocessing}
%-------------------------------
%
